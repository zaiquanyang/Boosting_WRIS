...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # cls_pos = self.positional_embedding[0:1, :]
        # spatial_pos = F.interpolate(self.positional_embedding[1:,].reshape(1, self.spacial_dim, self.spacial_dim, 768).permute(0, 3, 1, 2), size=(H, W), mode='bilinear')
        ...
        # cls_feats = x[:, 0, :]
        # spa_feats = x[:, 1:, :].permute(0, 2, 1).reshape(x.shape[0], -1, H, W)
        # return cls_feats, spa_feats 
...
class CLIP(nn.Module):
    ...
    def encode_text(self, text):
        ...
        hidden = x[torch.arange(x.shape[0]), text.argmax(dim=-1)] @ self.text_projection
...
def build_model(state_dict: dict, txt_length=40):
