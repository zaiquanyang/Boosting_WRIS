...
class bilateral_prompt(nn.Module):
    ...
    def forward_Qt_Kt(self, lan, sub_lan):
        
        # sub_lan = sub_lan.transpose(1, 2)
        B, _, C = sub_lan.shape
        # lan = lan.transpose(1, 2)
        B, N, C = lan.shape   # B x 1 x C
        Ci = lan.shape[-1]

        Qt, Kt, Vt = self.t_proj1(lan), self.t_proj2(sub_lan), self.t_proj3(sub_lan)
        
        Kt = Kt.reshape(B, C, -1)
        At = F.softmax(Qt.matmul(Kt) / math.sqrt(Ci), dim=2)  
        
        Vt = Vt.reshape(B, C, -1).transpose(1, 2)
        new_lan = At.matmul(Vt)  

        new_lan = self.t_output(new_lan)

        return new_lan
    
    def forward_Qt(self, vis, lan):

        lan = lan.transpose(1, 2)
        B, N, C = lan.shape 
        if B > vis.shape[0]:
            vis = torch.cat([vis, vis], dim=0)
        else:
            assert B == vis.shape[0]
        # B, C, H, W = vis.shape
        Ci = lan.shape[-1]
        
        Qt, Kv, Vv = self.t_proj1(lan), self.v_proj2(vis), self.v_proj3(vis)
        # breakpoint()
        Kv = Kv.reshape(B, C, -1)
    ...
    def forward_Qv(self, vis, lan):
        ...
        Qv, Kt, Vt = self.v_proj1(vis), self.t_proj2(lan), self.t_proj3(lan)
